...
class AppStore extends Component {
  state = {currentActiveTab: tabsList[0], searchedResults: [...appsList]}

  tabSelected = id => {
    const activeTab = tabsList.find(eachTab => {
      const {tabId} = eachTab
      return id === tabId
    })
    this.setState({currentActiveTab: activeTab})
  }

  getTheSearchResults = event => {
    const userInput = event.target.value
    const results = appsList.filter(eachApp => {
      const app = eachApp.appName.toLowerCase()
      return app.includes(userInput)
    })
    this.setState({searchedResults: results})
  }

  render() {
    const {currentActiveTab, searchedResults} = this.state
    ...
    return (
      <div className="bg-container">
        ...
        <div className="search-container">
          <input
            onChange={this.getTheSearchResults}
            className="input"
            type="search"
            placeholder="Search"
          />
          ...
          {tabsList.map(eachApp => {
            ...
            return (
              <TabItem
                ...
                isActive={currentActiveTab.tabId === tabId}
              />
            )
          })}
        </ul>
        ...
        <ul className="apps-container">
          {searchedResults.map(eachApp => {
            if (eachApp.category === tabId) {
              return (
                <AppItem
                  key={eachApp.tabId}
                  id={eachApp.tabId}
                  appImgUrl={eachApp.imageUrl}
